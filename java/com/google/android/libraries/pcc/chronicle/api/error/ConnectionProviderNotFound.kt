...
/** Implies that no [ConnectionProvider] was found which could fulfill the [request]. */
class ConnectionProviderNotFound(request: ConnectionRequest<*>) :
  ChronicleError(constructMessage(request)) {
  companion object {
    private fun constructMessage(request: ConnectionRequest<*>): String {
      return "No connectionProvider found for request: $request"
    }
  }
}
