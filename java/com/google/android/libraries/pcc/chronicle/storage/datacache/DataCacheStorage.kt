...
interface DataCacheStorage {
   ...
   */
  fun <T> put(cls: Class<out T>, entity: WrappedEntity<T>): Boolean

  /**
   * Removes stored instance with data type [T] and identified by [id].
   *
   * Returns the removed stored [WrappedEntity] instance, null if no instance is found with
   * [id] for type [T]
   */
  fun <T> remove(cls: Class<T>, id: String): WrappedEntity<T>?

  /** Returns all stored [WrappedEntity] instances for a specific data type [T]. */
  fun <T> all(cls: Class<T>): List<WrappedEntity<T>>

  /** Returns map of <EntityId, [WrappedEntity]> for instances for a specific data type [T]. */
  fun <T> allAsMap(cls: Class<T>): Map<String, WrappedEntity<T>>
   ...
   * @return The number of entities deleted
   */
  fun <T> purgeEntitiesForPackage(cls: Class<T>, packageName: String): Int
  ...
  fun <T> purgeEntitiesWhere(cls: Class<T>, predicate: (WrappedEntity<*>) -> Boolean): Int
